...
AI coding:
As an example I ran the following prompt through the most recent Claude Opus model (4.6), and varied the "thinking effort" setting across its four levels, from "low" to "max":
...
[](#thinking-fig) shows the timing results for this experiment.

```{figure} images/opus_effort_comparison.png
:label: thinking-fig
:align: center
:width: 600

The relative time taken for responses of Claude Opus 4.6 at different levels of thinking effort on the prompt "Why are there no animals with three legs?", averaged across 10 runs at each level.
```

This results in [](#thinking-fig) show that the response time was substantially higher for the low/medium thinking levels compared the high/max levels; the number of tokens was also substantially lower, meaning lower costs. It's also interesting to then compare the outputs at the different levels, which I did by providing samples of responses (with one from each effort level) and asking the Claude Sonnet 4.5 model to rank them in order of their quality.  These ranks were compared to the effort level rank; the average correlation across 20 samples was 0.5, showing that thinking effort does bear at least some relationship to the quality of the responses.  The utility of added thinking will depend on the nature of the problem, so it may be useful to ass

I also discovered an interesting phenomenon when I asked the LLM to give each response a grade on a 1-100 scale.  What I saw that a large proportion of the responses across effort levels were given a grade of 82.  This apparently is a known phenomenon with LLMs, known as the "B+ attractor", and is just another example that it's important not to take LLM outputs at face value.
