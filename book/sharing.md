...
Machine-readable metadata files such as `CITATION.cff` and `codemeta.json` are the citation-and-identity end of what the STAMPED framework calls the **A**ctionability principle: rather than describing in prose how the software should be cited and identified, they let humans and tools (and, increasingly, AI agents) act on the information directly.
